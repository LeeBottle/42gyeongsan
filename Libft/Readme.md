...
노드의 'content'는 주어진 파라미터로 초기화되고, 'next'는 NULL로 설정됩니다.  

● ft_lstadd_front  
새로운 노드를 리스트의 앞에 추가하는 함수입니다.  
리스트의 첫 번째 노드를 갱신합니다.  

● ft_lstsize  
리스트의 길이를 반환하는 함수입니다.  
리스트의 노드 개수를 세어서 반환합니다.  

● ft_lstlast  
리스트의 마지막 노드를 반환하는 함수입니다.  
리스트의 마지막 노드를 찾습니다.  
...
● ft_lstdelone  
특정 노드를 삭제하는 함수입니다.  
노드의 'content'는 주어진 'del' 함수로 삭제하고, 노드 자체를 free합니다.  

● ft_lstclear  
리스트의 모든 노드를 삭제하는 함수입니다.  
'del' 함수로 각 노드의 'content'를 삭제하고, 모든 노드를 free한 후 리스트의 포인터를 NULL로 설정합니다.  

● ft_lstiter  
리스트의 각 노드를 순차적으로 탐색하여, 주어진 함수 'f'를 각 노드의 'content'에 적용하는 함수입니다.  

● ft_lstmap  
리스트의 각 노드에 함수 'f'를 적용하여 새로운 리스트를 생성하는 함수입니다.  
새로운 리스트의 노드를 반환하며, 메모리 할당에 실패할 경우 NULL을 반환합니다.  
'del' 함수는 필요 시 노드의 'content'를 삭제하는 데 사용됩니다.